...
impl ToolProvisioner for CursorProvisioner {
    fn tool_id(&self) -> ToolId { ToolId::Cursor }

    fn detect(&self) -> DetectionResult {
        match &self.paths {
            Some(paths) => detection::detect_tool(ToolId::Cursor, paths),
            None => DetectionResult { tool: ToolId::Cursor, detected: false, methods: vec![], version: None, config_paths: vec![] },
        }
    }
    ...
    fn preview(&self, config: &McpInjectionConfig) -> Result<ProvisionPreview, ProvisioningError> {
        ...
        if let Some(mcp_path) = paths.mcp_config_path(ToolId::Cursor) {
        ...
        if let Some(skill_path) = paths.skill_path(ToolId::Cursor) {
            changes.push(FileChange {
                path: skill_path,
                change_type: FileChangeType::CreateFile,
                description: "Create Cursor rule file with wallet instructions".into(),
                diff: Some(content::cursor_rule_content()),
            });
        }

        Ok(ProvisionPreview { tool: ToolId::Cursor, changes })
    }
    ...
    fn provision(
        ...
        if let Some(mcp_path) = paths.mcp_config_path(ToolId::Cursor) {
            ...
            let backup_path = if mcp_path.exists() {
                Some(backup_mgr.backup_file(backup_dir, "cursor", &mcp_path)?.0)
            } else { None };
        ...
        if let Some(skill_path) = paths.skill_path(ToolId::Cursor) {
            let sha256_before = if skill_path.exists() {
                Some(backup::sha256_hex(&std::fs::read(&skill_path)?))
            } else { None };

            let backup_path = if skill_path.exists() {
                Some(backup_mgr.backup_file(backup_dir, "cursor", &skill_path)?.0)
            } else { None };

            let skill_content = content::cursor_rule_content();
            ...
            files_modified.push(ModifiedFile {
                ...
                sha256_after: config_writer::sha256_hex(skill_content.as_bytes()), created_new,
            });
        }

        Ok(ProvisionResult { tool: ToolId::Cursor, success: true, files_modified, error: None, needs_restart: false })
    }
    ...
    fn unprovision(&self) -> Result<UnprovisionResult, ProvisioningError> {
        ...
        Ok(UnprovisionResult {
            tool: ToolId::Cursor, success: true, files_restored, files_deleted,
            error: None, strategy_used: RollbackStrategy::SurgicalRemoval,
        })
    }
    ...
    fn verify(&self, _expected_version: &str) -> VerificationResult {
        let paths = match &self.paths {
            Some(p) => p,
            None => return VerificationResult { tool: ToolId::Cursor, status: VerificationStatus::Missing, details: vec![] },
        };
...
mod tests {
    ...
    fn make_provisioner(home: &std::path::Path) -> CursorProvisioner {
        CursorProvisioner::with_paths(PlatformPaths::with_home(home.to_path_buf()))
    }
    ...
    fn tool_id_returns_correct_id() {
        ...
        assert_eq!(p.tool_id(), ToolId::Cursor);
    }
    ...
    fn preview_returns_expected_files() {
        ...
        assert_eq!(preview.tool, ToolId::Cursor);
        assert_eq!(preview.changes.len(), 2);
        assert!(preview.changes[0].path.to_string_lossy().contains("mcp.json"));
        assert!(matches!(preview.changes[0].change_type, FileChangeType::MergeJsonKey));
        assert!(preview.changes[1].path.to_string_lossy().contains("tally-wallet.mdc"));
    ...
    fn provision_creates_expected_files() {
        ...
        assert_eq!(result.tool, ToolId::Cursor);

        let mcp_path = tmp.path().join(".cursor").join("mcp.json");
        assert!(mcp_path.exists());
        let contents = std::fs::read_to_string(&mcp_path).unwrap();
        assert!(contents.contains("tally-wallet"));

        let skill_path = tmp.path().join(".cursor").join("rules").join("tally-wallet.mdc");
