...
impl ToolProvisioner for WindsurfProvisioner {
    fn tool_id(&self) -> ToolId { ToolId::Windsurf }

    fn detect(&self) -> DetectionResult {
        match &self.paths {
            Some(paths) => detection::detect_tool(ToolId::Windsurf, paths),
            None => DetectionResult { tool: ToolId::Windsurf, detected: false, methods: vec![], version: None, config_paths: vec![] },
        }
    }
    ...
    fn preview(&self, config: &McpInjectionConfig) -> Result<ProvisionPreview, ProvisioningError> {
        ...
        if let Some(mcp_path) = paths.mcp_config_path(ToolId::Windsurf) {
            let entry = content::mcp_json_entry(config);
            changes.push(FileChange {
                path: mcp_path, change_type: FileChangeType::MergeJsonKey,
        ...
        if let Some(skill_path) = paths.skill_path(ToolId::Windsurf) {
            changes.push(FileChange {
                path: skill_path, change_type: FileChangeType::CreateFile,
                description: "Create wallet instructions file".into(),
                diff: Some(content::standalone_skill_content()),
            });
        }

        Ok(ProvisionPreview { tool: ToolId::Windsurf, changes })
    }
    ...
    fn provision(&self, config: &McpInjectionConfig, backup_dir: &Path) -> Result<ProvisionResult, ProvisioningError> {
        ...
        if let Some(mcp_path) = paths.mcp_config_path(ToolId::Windsurf) {
            ...
            let backup_path = if mcp_path.exists() { Some(backup_mgr.backup_file(backup_dir, "windsurf", &mcp_path)?.0) } else { None };
            ...
            files_modified.push(ModifiedFile {
                path: mcp_path, change_type: FileChangeType::MergeJsonKey, backup_path, sha256_before,
        ...
        if let Some(skill_path) = paths.skill_path(ToolId::Windsurf) {
            ...
            let backup_path = if skill_path.exists() { Some(backup_mgr.backup_file(backup_dir, "windsurf", &skill_path)?.0) } else { None };

            let content = content::standalone_skill_content();
            config_writer::create_standalone_file(&skill_path, &content)?;

            let created_new = sha256_before.is_none();
            files_modified.push(ModifiedFile {
                path: skill_path, change_type: FileChangeType::CreateFile, backup_path, sha256_before,
                sha256_after: config_writer::sha256_hex(content.as_bytes()), created_new,
            });
        }

        Ok(ProvisionResult { tool: ToolId::Windsurf, success: true, files_modified, error: None, needs_restart: false })
    }
    ...
    fn unprovision(&self) -> Result<UnprovisionResult, ProvisioningError> {
        ...
        Ok(UnprovisionResult { tool: ToolId::Windsurf, success: true, files_restored, files_deleted, error: None, strategy_used: RollbackStrategy::SurgicalRemoval })
    }

    fn verify(&self, _expected_version: &str) -> VerificationResult {
        let paths = match &self.paths { Some(p) => p, None => return VerificationResult { tool: ToolId::Windsurf, status: VerificationStatus::Missing, details: vec![] } };
            ...
            if mcp_path.exists() {
                if let Ok(contents) = std::fs::read_to_string(&mcp_path) {
                    if let Ok(doc) = serde_json::from_str::<serde_json::Value>(&contents) {
                        let has_key = doc.get("mcpServers").and_then(|s| s.get(content::MCP_SERVER_KEY)).is_some();
                        let status = if has_key { VerificationStatus::Intact } else { all_intact = false; VerificationStatus::Missing };
        ...
        VerificationResult { tool: ToolId::Windsurf, status: if all_intact { VerificationStatus::Intact } else { VerificationStatus::Missing }, details }
    }
...
mod tests {
    ...
    fn make_provisioner(home: &std::path::Path) -> WindsurfProvisioner {
        WindsurfProvisioner::with_paths(PlatformPaths::with_home(home.to_path_buf()))
    }
    ...
    fn tool_id_returns_correct_id() {
        ...
        assert_eq!(p.tool_id(), ToolId::Windsurf);
    }
    ...
    fn preview_returns_expected_files() {
        ...
        assert_eq!(preview.tool, ToolId::Windsurf);
        assert_eq!(preview.changes.len(), 2);
        assert!(preview.changes[0].path.to_string_lossy().contains("mcp_config.json"));
    ...
    fn provision_creates_expected_files() {
        ...
        assert_eq!(result.tool, ToolId::Windsurf);

        let mcp_path = tmp.path().join(".codeium").join("windsurf").join("mcp_config.json");
        assert!(mcp_path.exists());
        let contents = std::fs::read_to_string(&mcp_path).unwrap();
        assert!(contents.contains("tally-wallet"));

        let skill_path = tmp.path().join(".windsurf").join("rules").join("tally-wallet.md");
