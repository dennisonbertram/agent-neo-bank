...
pub struct CopilotProvisioner { paths: Option<PlatformPaths> }

impl CopilotProvisioner {
...
impl ToolProvisioner for CopilotProvisioner {
    fn tool_id(&self) -> ToolId { ToolId::Copilot }

    fn detect(&self) -> DetectionResult {
        match &self.paths { Some(paths) => detection::detect_tool(ToolId::Copilot, paths), None => DetectionResult { tool: ToolId::Copilot, detected: false, methods: vec![], version: None, config_paths: vec![] } }
    }
    ...
    fn preview(&self, _config: &McpInjectionConfig) -> Result<ProvisionPreview, ProvisioningError> {
        ...
        // Note: Copilot MCP uses VS Code settings.json or a dedicated mcp.json
        // For now, we focus on the instruction file since MCP for Copilot is still evolving
        if let Some(skill_path) = paths.skill_path(ToolId::Copilot) {
            changes.push(FileChange {
                path: skill_path, change_type: FileChangeType::AppendMarkdownSection,
                description: "Append wallet instructions with sentinel markers".into(),
                diff: Some(content::copilot_instructions_content()),
            });
        }

        Ok(ProvisionPreview { tool: ToolId::Copilot, changes })
    }
    ...
    fn provision(&self, config: &McpInjectionConfig, backup_dir: &Path) -> Result<ProvisionResult, ProvisioningError> {
        ...
        // Instruction file with sentinel markers
        if let Some(skill_path) = paths.skill_path(ToolId::Copilot) {
            let sha256_before = if skill_path.exists() { Some(backup::sha256_hex(&std::fs::read(&skill_path)?)) } else { None };
            let backup_path = if skill_path.exists() { Some(backup_mgr.backup_file(backup_dir, "copilot", &skill_path)?.0) } else { None };
        ...
        Ok(ProvisionResult { tool: ToolId::Copilot, success: true, files_modified, error: None, needs_restart: false })
    }
    ...
    fn unprovision(&self) -> Result<UnprovisionResult, ProvisioningError> {
        ...
        Ok(UnprovisionResult { tool: ToolId::Copilot, success: true, files_restored, files_deleted: vec![], error: None, strategy_used: RollbackStrategy::SurgicalRemoval })
    }

    fn verify(&self, _expected_version: &str) -> VerificationResult {
        let paths = match &self.paths { Some(p) => p, None => return VerificationResult { tool: ToolId::Copilot, status: VerificationStatus::Missing, details: vec![] } };
        ...
        VerificationResult { tool: ToolId::Copilot, status: if all_intact { VerificationStatus::Intact } else { VerificationStatus::Missing }, details }
    }
...
mod tests {
    ...
    fn make_provisioner(home: &std::path::Path) -> CopilotProvisioner {
        CopilotProvisioner::with_paths(PlatformPaths::with_home(home.to_path_buf()))
    }
    ...
    fn tool_id_returns_correct_id() {
        ...
        assert_eq!(p.tool_id(), ToolId::Copilot);
    }
    ...
    fn preview_returns_expected_files() {
        ...
        assert_eq!(preview.tool, ToolId::Copilot);
        assert_eq!(preview.changes.len(), 1);
        assert!(preview.changes[0].path.to_string_lossy().contains("copilot-instructions.md"));
        assert!(matches!(preview.changes[0].change_type, FileChangeType::AppendMarkdownSection));
    }
    ...
    fn provision_creates_expected_files() {
        ...
        assert_eq!(result.tool, ToolId::Copilot);

        let skill_path = tmp.path().join(".github").join("copilot-instructions.md");
    ...
    fn unprovision_removes_traces() {
        ...
        // File should still exist but without sentinel markers
        let skill_path = tmp.path().join(".github").join("copilot-instructions.md");
        if skill_path.exists() {
            let contents = std::fs::read_to_string(&skill_path).unwrap();
            assert!(!contents.contains("TALLY_WALLET_START"));
